 ...
 */
function getPulumiComponents(moduleExports) {
...
exports.getPulumiComponents = getPulumiComponents;
class ComponentProvider {
    constructor(options) {
        ...
        this.componentConstructors = options.components.reduce((acc, component) => {
    ...
    async getSchema() {
        const analyzer = new analyzer_1.Analyzer(this.path, this.name, this.packageJSON, new Set(Object.keys(this.componentConstructors)));
        const { components, typeDefinitions, packageReferences } = analyzer.analyze();
        const schema = schema_1.generateSchema(this.name, this.version, this.packageJSON.description, components, typeDefinitions, packageReferences, this.namespace);
...
let isHosting = false;
function componentProviderHost(options) {
    if (isHosting) {
